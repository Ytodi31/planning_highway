    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = std::hypot(actor_center_x - circle_center_x, actor_center_y  - circle_center_y);  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = main_goal.location.x - spiral.back().x;  // <- Update
  auto delta_y = main_goal.location.y - spiral.back().y;  // <- Update
  auto delta_z = main_goal.location.z - spiral.back().z;  // <- Update
